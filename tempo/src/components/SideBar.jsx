...
import { BsHeart, BsMenuButton } from "react-icons/bs";
import { BiHome, BiLogOut, BiSearch, BiSearchAlt } from "react-icons/bi";
...
import { MdMic, MdMicExternalOff } from "react-icons/md";
...
const SideBar = () => {
  ...
  return (
    <aside className="sticky left-0 top-0 h-screen w-[10%] bg-white bg-opacity-20 backdrop-blur-lg drop-shadow-lg">
      ...
      <nav className={` hidden md:flex flex-col items-start justify-around w-full h-full px-5 py-3`}>
        <h1 className="font-extrabold tracking-tighter leading-tight text-4xl">TEMPO</h1>
      ...
      <div className="font-medium text-xl flex items-start gap-1 flex-col">
          <Link to={'/'}><BiHome size={20}/></Link>
          <Link to={'/'} ><BiSearch size={20}/></Link>
        </div>
        <div className="font-medium text-xl flex items-start gap-1 flex-col">
          {/* <h1 className="text-sm font-bold text-white/80 ">Libariy</h1> */}
         
          <Link to={'/'}><MdMic size={20}/></Link>
          <Link to={'/'}><MdMic size={20}/></Link>
          <Link to={'/'}><MdMic size={20}/></Link>
        </div>
        <div className="font-medium text-xl flex items-start gap-1 flex-col">
          {/* <h1 className="text-sm font-bold text-white/80 ">My Account</h1> */}
          <Link to={'/'}><BsHeart size={20} /></Link>
          <Link to={'/'}><BiLogOut size={20}/></Link>
        </div>
     </nav>
    </aside>
  );
